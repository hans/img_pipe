...
# Date Last Edited: March 11, 2025
#
...
class freeCoG:
    ...
    def __init__(
        ...
        add_to_db : bool, optional
            Whether or not to add to the django database.
        ...
        if os.path.isfile(surf_file):
            ...
            self.pial_surf_file['lh'] = os.path.join(
                self.subj_dir, self.subj, 'Meshes', 'lh_pial_trivert.mat')
            self.pial_surf_file['rh'] = os.path.join(
                self.subj_dir, self.subj, 'Meshes', 'rh_pial_trivert.mat')
        ...
        if not os.path.isdir(os.path.join(self.subj_dir, self.subj)):
            print('No such subject directory as %s' %
                  (os.path.join(self.subj_dir, self.subj)))
            ans = input(
                'Would you like to create a new subject directory? (y/N): ')
            if ans.upper() == 'Y' or ans.upper() == 'YES':
                ...
                raise NameError(
                    'No such subject directory as %s' %
                    (os.path.join(
                        subj_dir, subj)))
        ...
        # Flag for whether this patient will be added to the database
        self.add_to_db = add_to_db

        # Add some things to the django database
        if self.add_to_db:
            try:
                print("Linking to database object for %s" % (self.subj))
                self.db_obj = Patient.objects.get(patient_uid=self.subj)

            except Patient.DoesNotExist:
                print("Creating patient in the Django database")
                self.db_obj = Patient(patient_uid=self.db_obj)
                self.db_obj.save()
                print(
                    "Auto-generating information... please go to web interface to add details")
                autogen_tag = PatientPropertyDefinition.objects.get(
                    property_uid='auto')
                autogen_prop = PatientPropertyRecord(
                    patient=self.db_obj, property_type=autogen_tag)
                autogen_prop.save()

            try:
                print("Finding entry for %s in ImagingMaster" % (self.subj))
                self.img_set = ImagingMaster.objects.get(patient=self.db_obj)
            except BaseException:
                print("Creating entry for %s in ImagingMaster" % (self.subj))
                self.img_set = ImagingMaster(patient=self.db_obj)
                self.img_set.save()
    ...
    def get_recon(self, flag_T3='-3T', openmp_flag='-openmp 12', gpu_flag=''):
        ...
        '''
        os.system('recon-all -subjid %s -sd %s -all %s %s %s' %
                  (self.subj, self.subj_dir, flag_T3, openmp_flag, gpu_flag))
        ...
        gyri_labels_dir = os.path.join(
            self.subj_dir, self.subj, 'label', 'gyri')
    ...
    def check_pial(self):
        ...
        os.system(
            "freeview --volume %s --surface %s --surface %s --viewport 'coronal'" %
            (brain_mri, lh_pial, rh_pial))
    ...
    def check_anatomy_freeview(self):
        ...
        aparc_aseg = os.path.join(
            self.subj_dir,
            self.subj,
            'mri',
            'aparc.a2009s+aseg.mgz')
        os.system(
            "freeview --volume %s:opacity=0.8 --volume %s:opacity=0.6 --volume %s:colormap=lut:opacity=0.5:visible=0 --viewport 'coronal'" %
            (brain_mri, elecs_CT, aparc_aseg))
    ...
    def make_dural_surf(self, radius=3, num_iter=30, dilate=0.0):
        ...
        for hem in hems:
            ...
            pial_surf = os.path.join(
                self.subj_dir, self.subj, 'surf', hem + '.pial')
            pial_fill_image = os.path.join(
                self.subj_dir, self.subj, 'surf', hem + '.pial.filled.mgz')
            if not os.path.isfile(pial_fill_image):
                os.system(
                    'mris_fill -c -r 1 %s %s' %
                    (pial_surf, pial_fill_image))
            ...
            if os.path.isfile(pial_fill_image):
                ...
                print(
                    "Creating outer surface for the filled pial surface %s" %
                    (pial_fill_image))
                make_outer_surf(
                    pial_surf,
                    pial_fill_image,
                    radius,
                    outfile)  # this is from ielu

                os.system(
                    'mris_extract_main_component %s %s-main' %
                    (outfile, outfile))
                dura_surf = os.path.join(
                    self.subj_dir, self.subj, 'surf', hem + '.dural')
                os.system(
                    'mris_smooth -nw -n %d %s-main %s' %
                    (num_iter, outfile, dura_surf))

                if dilate != 0:
                    print("Dilating surface by %d mm" % (dilate))
                    if dilate > 0:
                        print(
                            "Multiplying dilate value by -1 to get outward dilation")
                        dilate = -1 * dilate
                    os.system(
                        'mris_expand %s %d %s' %
                        (dura_surf, dilate, dura_surf))
            else:
    ...
    def mark_electrodes(self):
        ...
        of implantation.
        '''
        individual_elecs_dir = os.path.join(
            self.subj_dir, self.subj, 'elecs', 'individual_elecs')
    ...
    def convert_fsmesh2mlab(self, mesh_name='pial'):
        ...
        for h in hems:
            ...
            out_file = os.path.join(
                self.mesh_dir, '%s_%s_trivert.mat' %
                (h, mesh_name))
            out_file_struct = os.path.join(
                self.mesh_dir, '%s_%s_%s.mat' %
    ...
    def reg_img(
        ...
        print(
            "Computing registration from %s to %s using method %s" %
            (source_file, target_file, method))
        if method == "ants":
            ...
            ants.image_write(trans['warpedmovout'], outfile)
            ...
            aff = ct_to_mri_reg.optimize(reg_type.lower()).as_affine()
    ...
    def plot_overlay(self,
        ...
        image = nib.load(os.path.join(self.CT_dir, source))
        compare = nib.load(os.path.join(self.mri_dir, target))

        image = nib.orientations.apply_orientation(
            np.asarray(
                image.dataobj), nib.orientations.axcodes2ornt(
                nib.orientations.aff2axcodes(
                    image.affine)), ).astype(
            np.float32)
        compare = nib.orientations.apply_orientation(
            np.asarray(
                compare.dataobj), nib.orientations.axcodes2ornt(
                nib.orientations.aff2axcodes(
                    compare.affine)), ).astype(
            np.float32)
        ...
        for i, ax in enumerate(axes):
            ax.imshow(
                np.take(
                    image, [
                        image.shape[i] // 2], axis=i).squeeze().T, cmap="gray")
            ax.imshow(
    ...
    def interp_grid(self, nrows=16, ncols=16, grid_basename='hd_grid'):
        ...
        for i in np.arange(3):
            elecmatrix[corner_nums[0]:corner_nums[1] + 1,
                       i] = np.linspace(elecmatrix[corner_nums[0],
                                                   i],
                                        elecmatrix[corner_nums[1],
                                                   i],
                                        nrows)
            elecmatrix[corner_nums[2]:corner_nums[3] + 1,
                       i] = np.linspace(elecmatrix[corner_nums[2],
                                                   i],
                                        elecmatrix[corner_nums[3],
                                                   i],
                                        nrows)

        grid = np.arange(nchans).reshape((nrows, ncols), order='F')

        # Now fill in the rows using the new data from the leftmost and
        # rightmost columns
        for row in np.arange(nrows):
            for i in np.arange(3):
                elecmatrix[grid[row, :], i] = np.linspace(
                    elecmatrix[row, i], elecmatrix[row + (grid[0, -1] - grid[0, 0]), i], ncols)
    ...
    def extrap_grid(
        self, point_list=[
            (1, 1), (1, 5), (8, 1)], nrows=16, ncols=16, grid_basename='hd_grid'):
        ...
        # loop over x, y, and z coordinates
        for i in np.arange(3):
            linspacecol = np.linspace(
                elecmatrix[points_nums[0], i], elecmatrix[points_nums[1], i], point_list[1][1], retstep=True)
        ...
        for i in np.arange(3):
            linspacerow = np.linspace(
                elecmatrix[0, i], elecmatrix[points_nums[2], i], point_list[2][0], retstep=True)
            elecmatrix[grid[0, points_nums[0]:point_list[2][0]],
                       i] = linspacerow[0]
        ...
        for x in range(point_list[2][0] - 1, ncols):
            for i in np.arange(3):
                elecmatrix[grid[0, x],
                           i] = elecmatrix[grid[0, x - 1], i] + steprow[i]

        # Now fill in the rows using the new data from the leftmost and top
        for row in range(1, nrows):
            for col in range(1, ncols):
                for i in np.arange(3):
                    elecmatrix[grid[row, col],
                               i] = elecmatrix[grid[row - 1, col], i] + stepcol[i]

        grid_file = os.path.join(
            self.elecs_dir,
            'individual_elecs',
            grid_basename + '_orig.mat')
    ...
    def project_electrodes(
            self,
            elecfile_prefix='hd_grid',
            use_mean_normal=True,
            surf_type='dural',
            num_iter=30,
            dilate=0.0,
            grid=True,
        ...
        if surf_type == 'dural' and not os.path.isfile(dural_mesh):
            print(
                'Creating dural surface mesh, using %d smoothing iterations' %
                (num_iter))
        ...
        if use_mean_normal and grid:
            ...
            for vec in range(4):
                normal_vectors[vec] = normal_vectors[vec] / \
                    np.linalg.norm(normal_vectors[vec])

            # get all the normal vectors facing the same direction
            for vec in range(1, 4):
                angle = np.arccos(np.dot(normal_vectors[vec], normal_vectors[vec - 1]) / (
                    np.linalg.norm(normal_vectors[vec]) * np.linalg.norm(normal_vectors[vec - 1])))
                # flip the normal vector and see if angle between it and the
                # previous normal vector is smaller
                flipped_angle = np.arccos(np.dot((-1.0 * normal_vectors[vec]), normal_vectors[vec - 1]) / (
                    np.linalg.norm(normal_vectors[vec]) * np.linalg.norm(normal_vectors[vec - 1])))
        ...
        print(
            os.path.join(
                ...
                (self.hem,
                 surf_type)))
        cortex = scipy.io.loadmat(
            os.path.join(
                self.subj_dir, self.subj, 'Meshes', '%s_%s_trivert.mat' %
                (self.hem, surf_type)))
        ...
        print(direction)
        elecs_proj = project_electrodes_anydirection(
            tri, vert, elecmatrix, direction)
        scipy.io.savemat(
            os.path.join(
                self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s.mat' %
                (elecfile_prefix)), {
        ...
        if grid:
            ...
            preproc_dir = os.path.join(
                self.elecs_dir, 'individual_elecs', 'preproc')
    ...
    def get_clinical_grid(self, elecfile_prefix='hd_grid'):
        ...
        clingrid = scipy.io.loadmat(
            os.path.join(
                self.img_pipe_dir,
                'SupplementalFiles',
                'clingrid_inds.mat'))['inds'].ravel()
        ...
        # If we have a grid that is smaller than 256 channels, remove the
        # irrelevant clinical grid indices
        ...
        scipy.io.savemat(os.path.join(self.elecs_dir,
                                      'individual_elecs',
                                      'clinical_' + elecfile_prefix + '.mat'),
                         {'elecmatrix': clinicalgrid})

        return clinicalgrid

    def get_clinical_elecs_all(
            self,
        ...
        origelecs = scipy.io.loadmat(
            os.path.join(
                self.elecs_dir,
                elecfile_prefix +
                '.mat'))['elecmatrix']
        origlabels = scipy.io.loadmat(
            os.path.join(
                self.elecs_dir,
                elecfile_prefix +
                '.mat'))['anatomy']
        ...
        for r in origlabels:
            # This is the shortened electrode montage label
            short_label.append(r[0][0])
            # This is the long form electrode montage label
            long_label.append(r[1][0])
            # This is the label for grid, depth, or strip
            grid_or_depth.append(r[2][0])
                ...
                if masterlist[string] not in grid_shortnames:
                    ...
                    for grid in range(0, num_grids):
                        if masterlist[string] == grid_shortnames[grid] and clinicalgrids[grid].shape[0] > 0:
                            ...
                            newelecs = np.append(newelecs, coord, axis=0)
                            clinicalgrids[grid] = np.delete(
                                clinicalgrid, 0, axis=0)
        ...
        scipy.io.savemat(os.path.join(self.elecs_dir,
                                      'clinical_' + elecfile_prefix + '.mat'),
                         {'elecmatrix': newelecmatrix,
                          'eleclabels': neweleclabels})
    ...
    def get_subcort(self):
        ...
        os.system(
            os.path.join(
                ...
                'SupplementalScripts',
                'aseg2srf.sh') +
            ' -s "%s" -l "4 5 10 11 12 13 17 18 26 \
                 28 43 44  49 50 51 52 53 54 58 60 14 15 16" -d' %
            (self.subj))

        # get list of all .srf files and change fname to .asc
        srf_list = list(set([fname for fname in os.listdir(subjAscii_dir)]))
        asc_list = list(set([fname.replace('.srf', '.asc')
        ...
        for fname in srf_list:
            ...
            os.system(
                'mv %s %s' %
                (os.path.join(
                    subjAscii_dir, fname), os.path.join(
                    subjAscii_dir, new_fname)))
    ...
    def subcortFs2mlab(self, subcort, nuc):
        ...
        scipy.io.savemat(outfile, {'tri': subcort_tri, 'vert': subcort_vert})

        # convert inds to scipy mat
        subcort_inds = np.asmatrix(subcort_inds)
    ...
    def make_elecs_all(self, input_list=None, outfile=None):
        ...
        if input_list is not None:
            for device in input_list:
                short_name_prefix, long_name_prefix, elec_type, file_name = device[
                    0], device[1], device[2], device[3]
                if short_name_prefix.lower() == 'nan' or long_name_prefix.lower(
                ) == 'nan' or elec_type.lower() == 'nan' or file_name == 'nan':
                    num_empty_rows = file_name
                    elecmatrix_all.append(
                        np.ones((num_empty_rows, 3)) * np.nan)
                    short_names.extend(
                        [short_name_prefix for i in range(num_empty_rows)])
                    long_names.extend(
                        [long_name_prefix for i in range(num_empty_rows)])
                    elec_types.extend(
                        [elec_type for i in range(num_empty_rows)])
                else:
                    indiv_file = os.path.join(
                        self.elecs_dir, 'individual_elecs', file_name)
                    elecmatrix = scipy.io.loadmat(indiv_file)['elecmatrix']
                    num_elecs = elecmatrix.shape[0]
                    elecmatrix_all.append(elecmatrix)

                    short_names.extend([short_name_prefix + str(i)
                                       for i in range(1, num_elecs + 1)])
                    long_names.extend([long_name_prefix + str(i)
                                      for i in range(1, num_elecs + 1)])
                    elec_types.extend([elec_type for i in range(num_elecs)])
        else:  # interactive
            done = False
            while done == False:
                num_empty_rows = input(
                ...
                if len(num_empty_rows):
                    ...
                    short_name_prefix = input(
                        'What is the short name prefix (e.g. G, AD, HD)?\n')
                    short_names.extend(
                        [short_name_prefix for i in range(1, num_empty_rows + 1)])
                    long_name_prefix = input(
                        'What is the long name prefix (e.g. L256Grid, HippocampalDepth)?\n')
                    long_names.extend(
                        [long_name_prefix for i in range(1, num_empty_rows + 1)])
                    elec_type = input(
                    ...
                    try:
                        ...
                        file_name = input(
                            'Sorry, that file was not found. Please enter the correct filename of the device: \n ')
                        indiv_file = os.path.join(
                            self.elecs_dir, 'individual_elecs', file_name)
                        elecmatrix = scipy.io.loadmat(indiv_file)['elecmatrix']
                    num_elecs = elecmatrix.shape[0]
        ...
        scipy.io.savemat(
            label_outfile, {
                'eleclabels': eleclabels, 'elecmatrix': elecmatrix_all})
    ...
    def label_elecs(
        ...
        # atlas_surf is 'destrieux', in which case the more detailed labels are
        # used
        os.system(
            'mri_annotation2label --subject %s --hemi %s --surface pial %s --outdir %s' %
            (self.subj, self.hem, surf_atlas_flag, gyri_labels_dir))
        ...
        if elecfile_prefix == 'TDT_elecs_all' or elecfile_prefix == 'clinical_elecs_all' or elecs_all:
            ...
            indices = [
                i for i, x in enumerate(long_label) if (
            ...
            # Initialize the cell array that we'll store electrode labels in
            # later
            ...
            short_label = [i for j, i in enumerate(
                short_label) if j not in indices]
            long_label = [i for j, i in enumerate(
                long_label) if j not in indices]
            grid_or_depth = [i for j, i in enumerate(
                grid_or_depth) if j not in indices]
        ...
        # Use the surface label files to get which label goes with each surface
        # vertex
        ...
        vert_inds, nearest_verts = self.nearest_electrode_vert(
            cortex_verts, elecmatrix_new)
        ...
        if elecfile_prefix == 'TDT_elecs_all' or elecfile_prefix == 'clinical_elecs_all' or elecs_all:
            ...
            elec_labels[np.invert(isnotdepth), 3] = ''
        else:
            elec_labels = np.array(elec_labels_notdepth, dtype=object)
        print(
            'Saving electrode labels for surface electrodes to %s' %
            (elecfile_prefix))
        # added by BKD so that elec_mat_grid='hd_grid' works. It does not
        # contain elecmontage
        ...
        if np.any(np.invert(isnotdepth)
            ...
            # Find voxel CRS
            VoxCRS = np.dot(np.linalg.inv(affine),
                            elecs_ones.transpose()).transpose().astype(int)
            ...
            # Get the names of these labels using Freesurfer's lookup table
            # (LUT)
            ...
            for row in LUT:
                if len(
                        row) > 1 and row[0][0] != '#' and row[0][0] != '\\':  # Get rid of the comments
                    lname = row[1]
                    lab[int(row[0])] = lname
            ...
            for elec in np.arange(nchans):
                anatomy[elec] = lab[aparc_dat[VoxCRS[elec, 0],
                                              VoxCRS[elec, 1], VoxCRS[elec, 2]]]
                print("E%d, Vox CRS: [%d, %d, %d], Label #%d = %s" % (elec, VoxCRS[elec, 0], VoxCRS[elec, 1],
                      VoxCRS[elec, 2], aparc_dat[VoxCRS[elec, 0], VoxCRS[elec, 1], VoxCRS[elec, 2]], anatomy[elec]))
    ...
    def warp_all(
        ...
        elecfile = os.path.join(self.elecs_dir, elecfile_prefix + '.mat')
        elecfile_warped = os.path.join(
            self.elecs_dir,
            '%s_warped.mat' %
            (elecfile_prefix))
        elecfile_nearest_warped = os.path.join(
            self.elecs_dir, '%s_nearest_warped.mat' %
            (elecfile_prefix))
        elecfile_nearest_warped_text = os.path.join(
            self.elecs_dir, elecfile_prefix + '_nearest_warped.txt')
        elecfile_RAS_text = os.path.join(
            self.elecs_dir, elecfile_prefix + '_RAS.txt')

        if os.path.isfile(elecfile_warped):
            print(
                "The electrodes in %s have already been warped and are in %s" %
                (elecfile, elecfile_warped))
            return
        ...
        if 'depth' in orig_elecs['anatomy'][:, 2] and warp_depths:
            ...
            if not os.path.isfile(
                os.path.join(
                    ...
                    'combined_to' +
                    template +
                    '_elreg_afteraseg-norm.tm3d')):
                self.get_cvsWarp(template)
            else:
                print(
                    '%s registration file already created, proceeding to apply the depth warp' %
                    (os.path.join(
                        self.subj_dir,
                        self.subj,
                        'cvs',
            ...
            if not os.path.isfile(elecfile_nearest_warped):
                ...
                print(
                    "Depth warping has already been applied to the depth electrodes of %s and are in %s" %
        ...
        if warp_surface:
            ...
            if not os.path.isfile(
                ...
                print(
                    'Found %s, not running surface warp again' %
                    (os.path.join(
                        self.subj_dir,
                        self.subj,
                        'elecs',
                        elecfile_prefix +
                        '_surface_warped.mat')))
            elecfile_surface_warped = os.path.join(
                self.elecs_dir, elecfile_prefix + '_surface_warped.mat')
            surface_warps = scipy.io.loadmat(elecfile_surface_warped)
            surface_indices = np.array(list(set(np.where(orig_elecs['anatomy'][:, 2] != 'depth')[0]) & set(
                np.where(np.all(~np.isnan(orig_elecs['elecmatrix']), axis=1))[0])), dtype='int64')
            if surface_warps['elecmatrix'].size > 0:
                orig_elecs['elecmatrix'][surface_indices,
                                         :] = surface_warps['elecmatrix']

        # if both depth and surface warping have been done, create the combined
        # warp .mat file
        if warp_depths and warp_surface:
            scipy.io.savemat(elecfile_warped,
                             {'elecmatrix': orig_elecs['elecmatrix'],
                              'anatomy': orig_elecs['anatomy']})

            self.plot_recon_anatomy_compare_warped(
                elecfile_prefix=elecfile_prefix)
            ...
            if os.path.isfile(elecfile_nearest_warped_text):
                os.system(
                    'mv %s %s' %
                    (elecfile_nearest_warped_text, preproc_dir))
    ...
    def get_cvsWarp(self, template='cvs_avg35_inMNI152', openmp_threads=4):
        ...
        os.system(
            'mri_cvs_register --mov %s --template %s --nocleanup --openmp %d' %
            (orig, template, openmp_threads))
        print('cvsWarp COMPUTED')

    def apply_cvsWarp(self, elecfile_prefix='TDT_elecs_all',
        ...
        print(
            ':::: Computing RAS2Vox and saving %s txt file ::::' %
            elecfile_prefix)
        if len(elec.shape) > 1:
        ...
        template_brain_file = os.path.join(
            self.subj_dir, template_brain, 'mri', 'brain.mgz')
        morph_file = os.path.join(
            ...
            template_brain +
            '_elreg_afteraseg-norm.tm3d')
        nearest_warped_file = os.path.join(
            self.elecs_dir, elecfile_prefix + '_nearest_warped.txt')
        os.system(
            'applyMorph --template %s --transform %s tract_point_list %s %s nearest' %
            (template_brain_file, morph_file, RAS_text_file, nearest_warped_file))

        print(
            ':::: Computing Vox2RAS and saving %s_warped mat file ::::' %
            (elecfile_prefix))
        ...
        # This is for deleting the duplicate row that applyMorph produces for
        # some reason
        if (elecmatrix[-1, :] == elecmatrix[-2, :]).all():
            elecmatrix = elecmatrix[:-1, :]
        nearest_warped_matfile = os.path.join(
            self.elecs_dir, elecfile_prefix + '_nearest_warped.mat')
        scipy.io.savemat(
            nearest_warped_matfile, {
                'elecmatrix': elecmatrix, 'anatomy': anatomy})

        return elecmatrix

    # Method to perform surface warps
    def get_surface_warp(self, basename='TDT_elecs_all',
                         template='cvs_avg35_inMNI152'):
        ...
        elecfile = os.path.join(
            self.elecs_dir,
            '%s_surface_warped.mat' %
            (basename))
        ...
        if os.path.isfile(elecfile):
            ...
            elecmatrix = scipy.io.loadmat(
                os.path.join(
                    ...
                    basename +
                    '.mat'))['elecmatrix']
            anatomy = scipy.io.loadmat(
                os.path.join(
                    self.elecs_dir,
                    basename +
                    '.mat'))['anatomy']
            labelpath = os.path.join(self.subj_dir, self.subj, 'label')
            elecs_warped = self.compute_surface_warp(
                elecmatrix, anatomy, labelpath, basename, template)

            scipy.io.savemat(
                elecfile, {
                    'elecmatrix': np.array(elecs_warped), 'anatomy': anatomy})

            print(
                "Surface warp for %s complete. Warped coordinates in %s" %
                (self.subj, elecfile))

    def compute_surface_warp(
            self,
            elecmatrix,
            anatomy=None,
            labelpath='tmp/',
        ...
        if anatomy is not None:
            surface_indices = np.array(list(set(np.where(anatomy[:, 2] != 'depth')[0]) & set(
                np.where(np.all(~np.isnan(elecmatrix), axis=1))[0])), dtype='int64')
        else:
            surface_indices = range(len(elecmatrix))

        print("Finding nearest surface vertex for each electrode")
        vert_inds, nearest_verts = self.nearest_electrode_vert(
            cortex_src['vert'], elecmatrix[surface_indices, :])
        ...
        for c in np.arange(len(surface_indices)):
            ...
            labelname_nopath = '%s.%s.chan%03d.label' % (
                self.hem, basename, chan)
            ...
            with open(labelname, 'w') as fid:
                ...
                fid.write(
                    '#!ascii label  , from subject %s vox2ras=TkReg\n1\n' %
                    (self.subj))
                fid.write('%i %.9f %.9f %.9f 0.0000000' % (
                    vert_inds[chan], elecmatrix[chan, 0], elecmatrix[chan, 1], elecmatrix[chan, 2]))

            print("Warping ch %d" % (chan))
            trglabel = os.path.join(
                warped_labels_dir, '%s.to.%s.%s' %
            ...
            os.system(
                ...
                ' --trglabel ' +
                trglabel +
                ' --regmethod surface --hemi ' +
                self.hem +
                ' --trgsurf pial --paint 6 pial --sd ' +
                self.subj_dir)
            ...
            elecs_warped[c, :] = (
                [float(coord[1]), float(coord[2]), float(coord[3])])
            # else:
    ...
    def check_depth_warps(
        ...
        RAS_file = os.path.join(
            ...
            '%s_RAS.txt' %
            (elecfile_prefix))
        subj_elecs, subj_elecnums = self.get_depth_elecs(
            RAS_file, '\n', '\t', elecfile_prefix)
        ...
        nearest_warped_file = os.path.join(
            ...
            (elecfile_prefix))
        warped_elecs, warped_elecnums = self.get_depth_elecs(
            nearest_warped_file, '\n', ' ', elecfile_prefix)
    ...
    def apply_xfm(
            ...
            target_file='elecs/TDT_elecs_all_2tal.mat',
            file_type='elecs'):
        ...
        os.system(
            'mri_info --vox2ras-tkr %s > %s' %
            (orig_file, vox2rastk_file))
        ...
        if file_type == 'elecs':
            ...
            elecs_ones = np.hstack(
                (e['elecmatrix'], np.ones(
                    (e['elecmatrix'].shape[0], 1))))
            elecs_xfm = reduce(
                np.dot, [
                    xfm_mat, vox2ras, np.linalg.inv(vox2rastk), elecs_ones.T]).T
            ...
            vert_ones = np.hstack(
                (s['vert'], np.ones((s['vert'].shape[0], 1))))
            vert_xfm = reduce(
                np.dot, [
                    xfm_mat, vox2ras, np.linalg.inv(vox2rastk), vert_ones.T]).T
            print("Saving transformed coordinates as %s" % (target_file))
            scipy.io.savemat(os.path.join(self.patient_dir, target_file),
    ...
    def apply_transform(self, elecfile_prefix, reorient_file):
        ...
        reoriented_elecfile = os.path.join(
            self.elecs_dir, elecfile_prefix + '_reoriented.mat')
        scipy.io.savemat(
            reoriented_elecfile, {
                'elecmatrix': elecs_reoriented, 'anatomy': anatomy})
        print("Done.")
    ...
    def plot_elec(
        ...
        fs_lut = os.path.join(
            self.img_pipe_dir,
            'SupplementalFiles',
            'FreeSurferLUTRGBValues.npy')
        cmap = matplotlib.colors.ListedColormap(
            np.load(fs_lut)[:cvs_dat.max() + 1, :])

        lookupTable = os.path.join(
            self.img_pipe_dir,
            'SupplementalFiles',
            'FreeSurferLookupTable')
        ...
        cvs_vox_CRS = np.array([warped_coords[0] -
                                offset, warped_coords[1] -
                                offset, warped_coords[2] -
                                offset], dtype='int')
        ...
        plt.title('CVS brain ' +
                  lookup_dict[cvs_dat[cvs_vox_CRS[0], cvs_vox_CRS[1], cvs_vox_CRS[2]]], size=20)
        ...
        subj_vox_CRS = np.array([orig_coords[0] -
                                 offset, orig_coords[1] -
                                 offset, orig_coords[2] -
                                 offset], dtype='int')
        ...
        plt.title(self.subj + ' ' + \
                  lookup_dict[subj_dat[subj_vox_CRS[0], subj_vox_CRS[1], subj_vox_CRS[2]]], size=20)
        ...
        if cvs_dat[cvs_vox_CRS[0],
                   cvs_vox_CRS[1],
                   cvs_vox_CRS[2]] != subj_dat[subj_vox_CRS[0],
                                               subj_vox_CRS[1],
                                               subj_vox_CRS[2]]:
            result = 'MISMATCH'
            color = 'r'
    ...
    def get_elecs(self, elecfile_prefix='TDT_elecs_all', roi=None):
        ...
        if os.path.isfile(elecfile):
            ...
            if roi is not None:
                ...
                elecfile = scipy.io.loadmat(
                    os.path.join(
                        self.elecs_dir, '%s.mat' %
                        (elecfile_prefix)))
                return elecfile
            ...
            if hem == 'lh' or hem == 'rh':
                if template is None:
                    ...
                    template_file = os.path.join(
                        self.subj_dir, template, 'Meshes', hem + '_pial_trivert.mat')
                ...
                for h in ['lh', 'rh']:
                    if template is None:
                        ...
                        template_file = os.path.join(
                            self.subj_dir, template, 'Meshes', h + '_pial_trivert.mat')
            ...
            if roi_name in ('pial', 'rh_pial', 'lh_pial'):
                ...
                if roi_name in ('pial', 'lh_pial'):
                    ...
                    mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                        lh_pial['tri'], lh_pial['vert'], **kwargs)

                if roi_name in ('pial', 'rh_pial'):
                    rh_pial = self.get_surf(hem='rh', template=template)
                    mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                        rh_pial['tri'], rh_pial['vert'], **kwargs)

            else:
                subcort_dir = os.path.join(self.mesh_dir, 'subcortical')
                if os.path.isdir(subcort_dir) and '%s_subcort_trivert.mat' % (
                        roi_name) in os.listdir(subcort_dir):
                    roi_mesh = scipy.io.loadmat(
                        os.path.join(
                            subcort_dir,
                            '%s_subcort_trivert.mat' %
                            (roi_name)))
                else:
                    roi_mesh = scipy.io.loadmat(
                        os.path.join(
                ...
                mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                    roi_mesh['tri'], roi_mesh['vert'], **kwargs)
    ...
    def plot_recon_anatomy(
        ...
        if self.hem == 'lh' or self.hem == 'rh':
            ...
            mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                a['lh']['tri'], a['lh']['vert'], color=(
                    0.8, 0.8, 0.8), opacity=opacity)
            mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                a['rh']['tri'], a['rh']['vert'], color=(
                    0.8, 0.8, 0.8), opacity=opacity, new_fig=False)

        # Add the electrodes, colored by anatomical region
        elec_colors = np.zeros(
            (e['elecmatrix'].shape[0],
             e['elecmatrix'].shape[1]))
        ...
        for b in brain_areas:
            # Add relevant extra information to the label if needed for the
            # color LUT
            if b[0][0] != 'NaN':
                this_label = b[0]
                good_labels.append(this_label)
                if b[0][0:3] != 'ctx' and b[0][0:4] != 'Left' and b[0][0:
                                                                       5] != 'Right' and b[0][0:5] != 'Brain' and b[0] != 'Unknown':
                ...
                if this_label != '' and this_label != 'NaN':
                    if this_label not in cmap:
                        ...
                        el_color = matplotlib.cm.get_cmap('viridis').colors[int(
                            float(np.where(brain_areas == b)[0]) / float(len(brain_areas)))]
                    else:
    ...
    def plot_erps(
        ...
        for c in range(num_channels):
                ...
                if b != 'NaN':
                    ...
                    if this_label != '':
                        if this_label not in cmap:
                            # in case the label was manually assigned, and not
                            # found in the LUT colormap dictionary
                            erp_color = matplotlib.cm.get_cmap('viridis').colors[int(
                                float(np.where(brain_areas == b)[0]) / float(len(brain_areas)))]
                        else:
            ...
            x_array = np.array([elec_coord[0]
                               for i in range(num_timepoints)]) + label_offset
    ...
    def plot_recon_anatomy_compare_warped(
            self,
            template='cvs_avg35_inMNI152',
            elecfile_prefix='TDT_elecs_all',
            showfig=True,
        ...
        template_e = self.get_elecs(
            elecfile_prefix=elecfile_prefix + '_warped')

        subj_brain_width = np.abs(
            np.max(subj_brain['vert'][:, 1]) - np.min(subj_brain['vert'][:, 1]))
        template_brain_width = np.abs(
            np.max(template_brain['vert'][:, 1]) - np.min(template_brain['vert'][:, 1]))

        subj_e['elecmatrix'][:, 1] = subj_e['elecmatrix'][:, 1] - \
            ((subj_brain_width + template_brain_width) / 4)
        template_e['elecmatrix'][:, 1] = template_e['elecmatrix'][:,
                                                                  1] + ((subj_brain_width + template_brain_width) / 4)

        subj_brain['vert'][:, 1] = subj_brain['vert'][:, 1] - \
            ((subj_brain_width + template_brain_width) / 4)
        template_brain['vert'][:, 1] = template_brain['vert'][:,
                                                              1] + ((subj_brain_width + template_brain_width) / 4)

        # Plot the pial surface
        subj_mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
            subj_brain['tri'], subj_brain['vert'], color=(0.8, 0.8, 0.8))
        template_mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
            template_brain['tri'], template_brain['vert'], color=(
                0.8, 0.8, 0.8), new_fig=False)
        ...
        # Loop through unique brain areas and plot the electrodes in each brain
        # area
        for b in brain_areas:
            # Add relevant extra information to the label if needed for the
            # color LUT
    ...
    def make_roi_mesh(
        ...
        for label in label_list:
            this_label = os.path.join(
                self.patient_dir, 'label', 'gyri', '%s.%s.label' %
                (hem, label))
            f = open(this_label, 'r')
        ...
        tri_row_inds = np.sort(np.array(list(set(np.where(np.in1d(cortex['tri'][:, 0], vertnums))[0]) & set(np.where(
            np.in1d(cortex['tri'][:, 1], vertnums))[0]) & set(np.where(np.in1d(cortex['tri'][:, 2], vertnums))[0]))))
    ...
    def convert_bsmesh2mlab(self, mesh_name='pial.cortex'):
        ...
        # Load in the coordinate and transformation information from the T1
        # file to apply to the vertices
        ...
        # Convert the coordinate system and transformation to usable numpy
        # format
        ...
        # Divide by voxel size to convert to voxels and apply transformation
        # matrix
        ...
        # Save surfaces to cortex and triver .mat files for output for matlab
        # and python use
        out_file = os.path.join(
            self.mesh_dir,
            'bs_%s_trivert.mat' %
            (mesh_name))
        out_file_struct = os.path.join(
            self.mesh_dir, 'bs_%s_%s.mat' %
            (self.subj, mesh_name))
        scipy.io.savemat(out_file, {'tri': tri, 'vert': vert})
        cortex = {'tri': tri + 1, 'vert': vert}
        scipy.io.savemat(out_file_struct, {'cortex': cortex})

    def plot_all_surface_rois(
            self,
            bgcolor=(
                0,
                0,
        ...
        for roi in tqdm(rois):
            if 'ctx-' + self.hem + '-' + roi in color_dict:
                ...
                color = np.array(
                    color_dict['ctx-' + self.hem + '-' + roi]) / 255.
                ctmr_gauss_plot(
                    mesh['tri'],
                    mesh['vert'],
                    color=color,
    ...
    def animate_scene(
        ...
        if not keep_frames:
            png_files = os.path.join(
                movie_dir, 'frames', movie_name + '_*.png')
...
def _method_args_to_parser(method, parser):
    ...
    for arg_number in range(len(method_argnames)):
        if arg_number < required_args:
            ...
            parser.add_argument(
                '--' + method_argnames[arg_number],
                default=args_values[arg_number])
        elif isinstance(args_values[arg_number], bool):
            parser.add_argument(
                '--' + method_argnames[arg_number],
                type=_str2bool,
                default=args_values[arg_number])
...
if __name__ == "__main__":
    ...
    for free_method in free_methods:
        method_parser = subparsers.add_parser(
            free_method[0], help=free_method[1].__doc__)
    ...
    for key in method_args:
        if isinstance(method_args[key], str):
